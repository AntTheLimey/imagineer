...
2. **Sentences**: Use full, grammatically correct sentences
3. **Articles**: Use articles (a, an, the) when appropriate
4. **Pronoun clarity**: Do not refer to an object as "it" unless the object is in the same sentence
5. **No emojis**: Never use emojis unless explicitly requested
...
3. **Line wrapping**: Wrap all markdown files at 79 characters or less
...
1. **Explanatory text**: Include an explanatory sentence before code
...
3. **Code blocks**: Use fenced code blocks with language tags for multi-line code:
   ...
   SELECT * FROM code;
   ```
4. **Keywords**: Capitalize SQL keywords; lowercase variables
...
1. Verify all sentences are grammatically correct
