...
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # log_info prints an info-level message prefixed with a green "[INFO]" tag.

log_info() { echo -e "${GREEN}[INFO]${NC} $1"; }
# log_warn prints a yellow "[WARN]" prefixed warning message followed by the given text.
log_warn() { echo -e "${YELLOW}[WARN]${NC} $1"; }
# log_error prints an error message prefixed with "[ERROR]" in red to stdout.
log_error() { echo -e "${RED}[ERROR]${NC} $1"; }
# log_step prints a step-level message prefixed with a blue "[STEP]" tag.
log_step() { echo -e "${BLUE}[STEP]${NC} $1"; }

# cleanup stops the background API and client processes started by the script (if running) and logs progress.
cleanup() {
